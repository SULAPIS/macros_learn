...
pub fn get_string_literal(input: TokenStream) -> Result<String> {
    input
        ...
        .and_then(|v| match v {
            Literal::String(v) => Some(v.value().to_string()),
            _ => None,
        })
        .ok_or_else(|| anyhow!("only string literals"))
}
...
fn process_type(structs: &mut Vec<St>, k: &str, v: &Schema) -> Fd {
    ...
    match v.ty.as_str() {
        ...
        _ => panic!("unsupported schema type: {:?}", v),
    }
}
...
mod tests {
    ...
    #[test]
    fn schema_should_be_converted_to_st_2() {
    ...
    #[test]
    fn schema_render_should_render() {
